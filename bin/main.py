...
def calcOptimalAncientLvls(curAncients, siya, calcs):
    ...
    if calcs.alpha != 0:
        optAncients['Atman'] = int(
            ...
            - 1.416*log(4.0/3-exp(-0.013*int(curAncients['Atman'])))
            -6.613)
            )
        optAncients['Bubos'] = int(
            floor(2.8*log(siya)
            - 1.4*log(1+exp(-0.02*int(curAncients['Bubos'])))
            - 5.94)
            )
        optAncients['Chronos'] = int(
            floor(2.75*log(siya)
            - 1.375*log(2-exp(-0.034*int(curAncients['Chronos'])))
            - 5.1)
            )
        optAncients['Dogcog'] = int(
            floor(2.844*log(siya)
            - 1.422*log(1.0/99 + exp((-0.01)*int(curAncients['Dogcog'])))
            - 7.232)
            )
        optAncients['Dora'] = int(
            floor(2.877*log(siya)
            - 1.4365*log(100.0/99-exp(-0.002*int(curAncients['Dora'])))
            - 9.63)
            )
        optAncients['Fortuna'] = int(
            floor(2.875*log(siya)
            - 1.4375*log(10.0/9-exp(-0.0025*int(curAncients['Fortuna'])))
            - 9.3)
            )
        optAncients['Kuma'] = int(
            floor(2.844*log(siya)
            - 1.422*log(calcs.alpha)
            - 1.422*log(0.25 + exp(-0.01*int(curAncients['Kuma'])))
            - 7.014)
            )
